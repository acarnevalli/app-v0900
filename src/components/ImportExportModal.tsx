...
export const importProductsCSV = (csvText: string): CSVImportResult => {
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      headers.forEach((header, index) => {
        ...
        switch (header) {
          ...
          case 'cost':
          case 'price_cost':
            product.price_cost = parseFloat(value.replace(',', '.')) || 0;
            break;
          case 'unidade':
          case 'unit':
          case 'unidade_medida':
          ...
          case 'category':
            ...
            break;
          case 'descrição completa':
          case 'descricao completa':
          case 'description':
          ...
          case 'active':
          case 'status':
            product.active = value.toLowerCase() === 'sim' || value.toLowerCase() === 'ativo' || value === '1' || value.toLowerCase() === 'true';
            break;
          case 'estoque':
          case 'stock':
          case 'quantidade':
      ...
      if (!product.name || product.name.trim() === '') {
        warnings.push(`Linha ${i + 1}: Produto sem nome válido, pulando...`);
        continue;
      }
      ...
      if (!product.price_cost) product.price_cost = 0;

      products.push(product);
    }

    console.log(`Importação concluída: ${products.length} produtos importados, ${warnings.length} avisos`);
    ...
    if (products.length === 0) {
      return { 
        success: false, 
        error: 'Nenhum produto válido encontrado no arquivo. Verifique se o arquivo possui dados válidos.' 
      };
    }
