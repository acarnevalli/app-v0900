...
const SupplierModal: React.FC<SupplierModalProps> = ({ 
  ...
  const { addSupplier, updateSupplier } = useApp(); // Adicione updateSupplier aqui se disponível
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (supplier?.id) {
        // Se updateSupplier estiver disponível no contexto, use-o
        // await updateSupplier(supplier.id, cleanData);
        // Por enquanto, você pode implementar a lógica de atualização aqui
      } else {
        // Use a função addSupplier do contexto
        await addSupplier(cleanData);
        
        // Se precisar chamar onSuccess, passe o novo fornecedor
        if (onSuccess) {
          onSuccess({
            ...cleanData,
